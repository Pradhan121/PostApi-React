  ...
  const columns=[
   ...
   const[rowPage,setRowPage] =  React.useState(10);
    ...
    const handleChangeRowsPerPage=(event) => {
     ...
     useEffect(() => {
        fetch(`https://hammerhead-app-tzlph.ondigitalocean.app/customers?pageNo=${page}&rowPage=${rowPage}`)
         .then(resp => {
           return resp.json();
       }).then(resp => {
          setRow(resp.content);
   ...
   <div>
    <Paper sx={{ width: '90%',marginLeft:'5%',marginTop:'4%'}}>
      <TableContainer>
           ...
           <TableBody>
             ...
             .map((row,i) => {
                ...
                <TableRow key={i}>
                  ...
                  <TableCell>{row.customerPassword}</TableCell>
                 </TableRow>
                )
             })}
           </TableBody>
       ...
       <TablePagination rowPageOption={[5,10,25]}
           Component="div"
            page={page}
            rowpage={rowPage}
            count={rows.length}
